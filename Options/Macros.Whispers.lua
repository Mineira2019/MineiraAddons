...
function GMGenie.Macros.Whispers.optionsOnLoad()
    ...
    panel.name = "Macros de response";
    panel.parent = "GM Genie";

    InterfaceOptions_AddCategory(panel);

    getglobal(panel:GetName() .. "_Title"):SetText("Macros de response");
    getglobal(panel:GetName() .. "_SubText"):SetText("Aqui puedes agregar y editar los macros de response, los cuales tienes disponibles en la ventana Pj info o en el sub-menu de jugador");
													--Here you add and update whisper macros, which will be available from the ticket interface and the player context menus.
    GMGenie_Macros_Whispers_OptionsWindow_Info_Text:SetText("Nota: cada salto de linea en un macro de response será otro response mas (appendln). No es posible tener saltos de linea en un mismo response\n Nota: si el texto se vuelve rojo quiere decir que se ha sobre-pasado el limite de de caracteres en un mismo susurro, lo cual puedes arreglar\n separando con un salto de linea \nSugerencia: use NAME (en mayusculas sostenidas) para usar su nombre en él. use SUBJECT, para que sea remplazado la palabra según como haya definido el asunto en la macro.");
															--Note: every newline in the whisper macro will be a separate whisper. It is not possible to have newlines within one whisper.\nNote: If the text in the box turns red that means one of the lines (messages) has become too long and might get cut off when sending a whisper.\n\nTip: use NAME (in full caps) to use the players' name in the whisper text. Use SUBJECT (also in full caps), to get a pop up box where you can enter a subject for the message. SUBJECT in your macro will be replaced by this.
...
function GMGenie.Macros.Whispers.showOptions()
    InterfaceOptionsFrame_OpenToCategory("Macros de response");
end
