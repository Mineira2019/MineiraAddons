...
function GMGenie.Macros.Tele.optionsOnLoad()
    ...
    panel.name = "Macros de teletransportación";
    panel.parent = "GM Genie";

    InterfaceOptions_AddCategory(panel);

    getglobal(panel:GetName() .. "_Title"):SetText("Macros de teletransportación");
    getglobal(panel:GetName() .. "_SubText"):SetText("Aqui puedes agregar y editar los macros de teletranportación los cuales tenes disponible en la ventana de Pj info y el sub-menu de persoanje");
													--Here you add and update teleport macros, which will be available from the ticket interface and the player context menus.
    GMGenie_Macros_Tele_OptionsWindow_Info_Text:SetText("Sugerencia: use RECALL (en mayusculas sostenidas) Para regresar al jugador al punto donde estaba el jugador de haberle invocado un GM");
														--Tip: use RECALL (in full caps) as the location to return a player to wherever they were before they were teleported or summoned by a GM.
...
function GMGenie.Macros.Tele.showOptions()
    InterfaceOptionsFrame_OpenToCategory("Macros de teletransportación");
end
