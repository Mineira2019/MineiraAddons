...
function GMGenie.Macros.Mail.optionsOnLoad()
    ...
    panel.name = "Macros de mensajes por buzon";
    panel.parent = "GM Genie";

    InterfaceOptions_AddCategory(panel);

    getglobal(panel:GetName() .. "_Title"):SetText("Macros de mensajes por buzon");
    getglobal(panel:GetName() .. "_SubText"):SetText("Aqui puedes agregar y editar los macros de mensajes por buzon, los cuales tienes disponibles en la ventana de Pj info y los sub-menu de jugadores");
													--Here you add and update mail macros, which will be available from the ticket interface and the player context menus.
    GMGenie_Macros_Mail_OptionsWindow_Info_Text:SetText("Nota: cada salto de linea en un macro de mensaje por buzon lo separará en un nuevo mensaje. No es posible tener saltos de linea en un mensaje. \nNota 2: Si el texto se vuelve rojo quiere decir que esta sobre-pasado el limite de un mensaje; se puede arreglar con saltos de linea. \npara asi separarlo en varios mensajes \nSugerencia: use NAME (en mayusculas sostenidas) para que ello sea \nreemplazado con el nombre del jugador en el mensaje por buzon");
														--Note: every newline in the mail macro will be a separate mail. It is not possible to have newlines within one mail.\nNote: If the text in the box turns red that means one of the lines (messages) has become too long and might get cut off when sending a mail.\n\nTip: use NAME (in full caps) to use the players' name in the mail text.
...
function GMGenie.Macros.Mail.showOptions()
    InterfaceOptionsFrame_OpenToCategory("Macros de mensajes por buzon");
end
