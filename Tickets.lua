...
GMGenie.Tickets.PasoChat = 0;
GMGenie.Tickets.sizeRespond=0;
GMGenie.Tickets.iterationRespondLast=0
GMGenie.Tickets.iterationRespond=0;
GMGenie.Tickets.iterationRespondLastSubstract=0;
...
function GMGenie.Tickets.onLoad()
 ...
 --    Chronos.scheduleRepeating('QuestLoadTotal', 1, QuestLoadTotalBotonsb);
       Chronos.scheduleRepeating('QuestLoadTotalb', 0.01, QuestLoadTotalBotonsa);
...
function GMGenie.Tickets.updateView()
	...
	if GMGenie.Tickets.onlineTickets>=1 and GMGenie.Tickets.PasoChat==0 then
		GMGenie.showGMMessage("Hay Tickets");--Muestra en el chat cuando hay tickets abiertos (Show in chat if exist open tickets)
	end
...
function GMGenie.Tickets.loadComment(comment)
	...
	GMGenie.Tickets.currentTicket["comment"] = "";
    GMGenie.Tickets.showMessage();
end
...
function GMGenie.Tickets.comment(ticketId, comment)
    if GMGenie.Tickets.currentTicket["ticketId"] == ticketId then
		...
		GMGenie.showGMMessage("Comentario del ticket " .. GMGenie.Tickets.currentTicket["ticketId"]);
		GMGenie.showGMMessage(comment);
	end
		GMGenie.Tickets.currentTicket["comment"] = "";
        GMGenie.Tickets.showMessage();
        return true;
...
function GMGenie.Tickets.setComment()
	...
	GMGenie.showGMMessage("Ticket append al ticket " .. GMGenie.Tickets.currentTicket["ticketId"]);
	GMGenie.Tickets.iterationRespond=0;
	GMGenie.Tickets.iterationRespondLast=0
	
	GMGenie.Tickets.sizeRespond=string.len(GMGenie_Tickets_View_Comment:GetText());
	while(GMGenie.Tickets.iterationRespond<GMGenie.Tickets.sizeRespond) do
		GMGenie.Tickets.iterationRespondLastSubstract=0;

		
		GMGenie.Tickets.iterationRespondLast=GMGenie.Tickets.iterationRespond+230;
		GMGenie.Tickets.iterationRespondLastSubstract=0;
		while string.byte(string.sub(GMGenie_Tickets_View_Comment:GetText(),(GMGenie.Tickets.iterationRespondLast), (GMGenie.Tickets.iterationRespondLast)))~=32 do
			
			GMGenie.Tickets.iterationRespondLast=GMGenie.Tickets.iterationRespondLast-1;
			
			GMGenie.Tickets.iterationRespondLastSubstract=GMGenie.Tickets.iterationRespondLastSubstract+1;
			--parametro de seguridad
			if GMGenie.Tickets.iterationRespondLastSubstract>50 then
				break
			end
		end
		
		SendChatMessage(".ticket resp app " .. GMGenie.Tickets.currentTicket["ticketId"] .. " " .. string.sub(GMGenie_Tickets_View_Comment:GetText(), GMGenie.Tickets.iterationRespond, GMGenie.Tickets.iterationRespondLast), "GUILD");
		
		--mostrando resultado
		GMGenie.showGMMessage(string.sub(GMGenie_Tickets_View_Comment:GetText(), GMGenie.Tickets.iterationRespond, GMGenie.Tickets.iterationRespondLast))
		
		GMGenie.Tickets.iterationRespond=GMGenie.Tickets.iterationRespond+230-GMGenie.Tickets.iterationRespondLastSubstract;
